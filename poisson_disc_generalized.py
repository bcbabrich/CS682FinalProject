...
# generalized poisson disc sampling code taken from here
...
class Grid:
    ...
    of 'discs'
    """
    def __init__(self, r, desired_num_points, *size):
        self.r = r
    ...
    def poisson(self, seed, k=30):
        ...
        while self.active:
            ...
            # cap the number of samples so as to not go over desired_num_points
            # print('I am still active. len(self.samples):',len(self.samples))
            if (self.desired_num_points + 5) < len(self.samples) :
                #print('capped at ',len(self.samples))
                return self.samples
